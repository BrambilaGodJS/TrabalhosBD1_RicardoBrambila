...
select r.nome, c.nome, po.nome, pp.nome from Rotulo r INNER JOIN Cervejaria c ON r.cervejaria_cod = c.codigo INNER JOIN Pais po ON r.pais_origem_cod = po.codigo INNER JOIN Rotulo_Pais_Producao rpp ON r.codigo = rpp.rotulo_codigo INNER JOIN Pais pp ON rpp.pais_cod = pp.codigo AND r.temperatura_minima BETWEEN 0 AND 10;
...
SELECT p.nome, r.nome, c.nome FROM Pais p LEFT JOIN Rotulo r ON p.codigo = r.pais_origem_cod INNER JOIN Cervejaria c ON r.cervejaria_cod = c.codigo ;
...
SELECT COUNT(*) as quantidade, r.temperatura_minima, r.temperatura_maxima from (SELECT DISTINCT r.temperatura_minima, r.temperatura_maxima from Rotulo r);
...
SELECT e.nome from Estabelecimento e INNER JOIN Rotulo r ON e.rotulo_codigo = r.codigo AND r.teor_alcoolico = MAX(r.teor_alcoolico) AND r.nome like 'B%';

-- 15 - Listar o nome das pessoas que sгo proprietбrias de estabelecimentos, mas que nгo trabalham em nenhum estabelecimento.

SELECT p.nome from Pessoa p INNER JOIN Propriedade prop on p.codigo = prop.pessoa_cod AND p.codigo not in (SELECT t.pessoa_cod from Trabalha);

-- 16 - Retornar o nome dos estabelecimentos que vendem mais do que 10 rуtulos diferentes.
SELECT e.nome from Estabelecimento e INNER JOIN Rotulo_Estabelecimento re on e.codigo = re.estabelecimento_cod AND (SELECT COUNT(*) from Rotulo_Estabelecimento rere where e.codigo = rere.estabelecimento_cod) > 10;

-- 17 - Recuperar o nome, a faixa de preзo, a quantidade de rуtulos vendidos pelo estabelecimento que tкm mais do que 2 sommelier. Apresentar o resultado ordenado pela quantidade de rуtulos vendidos.
